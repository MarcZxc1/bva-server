...
const prisma_2 = __importDefault(require("../src/lib/prisma"));
 ...
 */
function generateSalesPattern(productIndex, days = 60, productData) {
    const sales = [];
    ...
    for (let day = 0; day < days; day++) {
        ...
        const dailySales = Math.max(0, Math.round(baseVelocity * weekendBoost * randomFactor));
...
async function main() {
    ...
    await prisma_2.default.sale.deleteMany();
    await prisma_2.default.inventoryLog.deleteMany();
    await prisma_2.default.inventory.deleteMany();
    await prisma_2.default.forecast.deleteMany();
    await prisma_2.default.campaign.deleteMany();
    await prisma_2.default.integration.deleteMany();
    await prisma_2.default.product.deleteMany();
    await prisma_2.default.shop.deleteMany();
    await prisma_2.default.user.deleteMany();
    // Create test users
    console.log("👤 Creating test users...");
    // Hash the password properly
    const hashedPassword = await bcrypt_1.default.hash("password123", 10);
    const adminUser = await prisma_2.default.user.create({
        data: {
    ...
    const testerUser = await prisma_2.default.user.create({
        data: {
            email: "tester@test.com",
            password: hashedPassword,
            name: "Test Seller",
            role: prisma_1.Role.SELLER,
        },
    });
    ...
    const shop = await prisma_2.default.shop.create({
        data: {
            name: "Main Store",
            ownerId: user.id,
        },
    ...
    for (let i = 0; i < SAMPLE_PRODUCTS.length; i++) {
        ...
        if (productData.nearExpiry) {
            expiryDate = new Date();
            expiryDate.setDate(expiryDate.getDate() + (3 + Math.floor(Math.random() * 5))); // 3-7 days from now
        }
        const product = await prisma_2.default.product.create({
            data: {
                shopId: shop.id,
                sku,
        ...
        await prisma_2.default.inventory.create({
            data: {
    ...
    for (let day = 0; day < salesCount; day++) {
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if (dailyQty > 0) {
                // Create sale record
                await prisma_2.default.sale.create({
                    data: {
                        shopId: shop.id,
                        platform: prisma_1.Platform.OTHER,
                        platformOrderId: `ORDER-${saleDate.toISOString().split("T")[0]}-${product.sku}-${Math.random().toString(36).substring(7)}`,
