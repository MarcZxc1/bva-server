...
const testCases = [
    {
        ...
        expectedStatus: 200,
        validate: (response) => {
            console.assert(response.data.success === true, "Success flag should be true");
            console.assert(response.data.data.strategy === "profit", "Strategy should be profit");
            console.assert(response.data.data.recommendations.length > 0, "Should have recommendations");
            console.assert(response.data.data.summary.totalCost <= 5000, "Total cost should not exceed budget");
            console.assert(response.data.data.summary.expectedProfit > 0, "Expected profit should be positive");
    ...
    },
    {
        ...
        validate: (response) => {
            console.assert(response.data.success === true, "Success flag should be true");
            console.assert(response.data.data.strategy === "volume", "Strategy should be volume");
            console.assert(response.data.data.recommendations.length > 0, "Should have recommendations");
            console.assert(response.data.data.summary.totalCost <= 3000, "Total cost should not exceed budget");
    ...
    },
    {
        ...
        validate: (response) => {
            console.assert(response.data.success === true, "Success flag should be true");
            console.assert(response.data.data.strategy === "balanced", "Strategy should be balanced");
            console.assert(response.data.data.recommendations.length > 0, "Should have recommendations");
            console.assert(response.data.data.summary.expectedROI > 0, "Expected ROI should be positive");
...
async function runTests() {
    ...
    for (const testCase of testCases) {
        ...
        console.log("-".repeat(60));
        try {
            const response = await axios_1.default.post(`${MAIN_SERVER_URL}/api/ai/restock-strategy`, testCase.payload, { validateStatus: () => true } // Don't throw on non-2xx status
            );
            // Check status code
            ...
            if (testCase.expectedStatus === 200) {
                ...
                console.log(`   Budget Used: ${data.summary.budgetUtilization.toFixed(1)}%`);
                console.log(`   Expected Profit: ₱${data.summary.expectedProfit.toFixed(2)}`);
...
async function healthCheck() {
    ...
    try {
        // Check main server
        const serverHealth = await axios_1.default.get(`${MAIN_SERVER_URL}/health`);
        console.log("✅ Main server is healthy");
        // Check ML service via main server
        const mlHealth = await axios_1.default.get(`${MAIN_SERVER_URL}/api/ai/restock-strategy/health`);
...
//# sourceMappingURL=restock-integration.test.js.map